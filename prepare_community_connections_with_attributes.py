...
class ConnectionDataPreparerWithAttributes:
    """准备社区连接可视化数据 - 包含详细社区属性"""

    def __init__(self, similarity_file, community_data_dir, output_file=None):
        """
        Parameters:
        -----------
        similarity_file : str
        ...
        if output_file:
            ...
            self.output_file = self.similarity_file.parent / "community_connections_with_attributes.json"

        # 过滤阈值
        self.min_overlap_size = 4
    ...
    def extract_community_attributes(self, df, community_id, level='level1'):
        ...
        dict: 包含社区详细属性的字典
        """
        column_name = f"community_{level}"
        ...
        # 筛选属于该社区的所有记录
        community_df = df[df[column_name].astype(str) == str(community_id)]

        if len(community_df) == 0:
            logging.warning(f"No records found for community {community_id} at {level}")
            return self._get_empty_attributes()
    ...
    def load_all_community_attributes(self, years):
        ...
        """
        logging.info("\n" + "="*80)
        logging.info("Loading community attributes for all years")
        logging.info("="*80)
        ...
        for year in sorted(years):
            ...
            # 检测可用的层级
            level_columns = [col for col in df.columns if col.startswith('community_level')]
            if not level_columns:
                logging.warning(f"No community level columns found in {year} data")
                continue

            # 默认使用level1
            level = 'level1'
            if 'community_level1' not in df.columns:
                level = level_columns[0].replace('community_', '')

            # 获取该年份所有社区ID
            column_name = f"community_{level}"
            community_ids = df[column_name].unique()

            logging.info(f"Processing {len(community_ids)} communities for year {year}")

            # 为每个社区提取属性
            for comm_id in community_ids:
                key = (int(year), str(comm_id))
                attributes = self.extract_community_attributes(df, comm_id, level)
    ...
    def prepare_data(self, df):
        ...
        """
        logging.info("\n" + "="*80)
        logging.info("Preparing visualization data with attributes")
        logging.info("="*80)
    ...
    def print_summary(self, data):
        ...
        print("\n" + "="*80)
        print("DATA SUMMARY - WITH DETAILED ATTRIBUTES")
        print("="*80)
    ...
    def run(self):
        """运行完整流程"""
        logging.info("\n" + "="*80)
        logging.info("COMMUNITY CONNECTION DATA PREPARATION - WITH ATTRIBUTES")
        logging.info("="*80)
        ...
        logging.info("\n✓ Data preparation with attributes completed!\n")
...
def main():
    ...
    try:
        ...
        if data:
            ...
            print(f"3. Explore communities with detailed attributes:")
